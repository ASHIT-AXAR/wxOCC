...
private:
    ...
    wxPanel* CreatewxOCCCntrl(wxWindow* parent = NULL);
    wxTextCtrl* CreateTextCtrl(const wxString& text = wxEmptyString);
    wxHtmlWindow* CreateHTMLCtrl(wxWindow* parent = NULL);
    wxAuiNotebook* CreateNotebook();
    wxOCCWindow* GetActiveDocument();
...
private:
    ...
    void OnCreatewxOCCCntrl(wxCommandEvent& evt);
...
bool MyApp::OnInit()
{
    ...
    wxFrame* frame = new MainFrame(NULL, wxID_ANY, wxT("wxOCC Sample"), wxDefaultPosition, wxSize(800, 600));
    frame->Show();
...
EVT_MENU(MainFrame::xID_FILE_NEW, MainFrame::OnCreatewxOCCCntrl)
    ...
    : wxFrame(parent, id, title, pos, size, style)
{
   ...
   // old_art = tb_main->GetArtProvider();
    ...
    m_mgr.AddPane(tb_view, wxAuiPaneInfo().Name(wxT("tb_view")).Caption(wxT("View")).ToolbarPane().Top());

    // add the toolbars to the manager
    m_mgr.AddPane(tb_main, wxAuiPaneInfo().Name(wxT("tb_main")).Caption(wxT("Main")).ToolbarPane().Top());
...
wxString MainFrame::GetIntroText()
{
    const char* text =
        "<html><body>"
        "<h3>Welcome to wxOCC</h3>"
        "<br/><b>Overview</b><br/>"
        "<p>wxOCC is an implementation of OpenCascade integrated into "
        ...
        "<p><b>Features</b></p>"
        "<p>With wxOCC, developers can create CAD programs using the Open Casecade framework."
        "with wxWidgets as the cross platform GUI Backend. This was originallty done as a proof of concept for the "
        "I.R.I.S. CAD "
        ...
        "<p><b>v. 0.3.0</b></p>"
        "<p>wxOCC 0.3.0, the demo is more matured and has added functionality."
        "<ul>"
...
wxPanel* MainFrame::CreatewxOCCCntrl(wxWindow* parent)
{
    if(!parent)
        parent = this;

    wxOCCWindow* ctrl = new wxOCCWindow(parent, wxPoint(0, 0), wxSize(-1, -1), 0);
...
wxAuiNotebook* MainFrame::CreateNotebook()
{
    ...
    ITEMCOUNT++;
    m_ntbk->AddPage(CreatewxOCCCntrl(m_ntbk), wxString::Format(wxT("newfile%i.wxocc"), ITEMCOUNT), false, page_bmp);

    m_mgr.AddPane(CreateHTMLCtrl(),
                  wxAuiPaneInfo()
                      .Name(wxT("Welcome"))
                      .Caption(wxT("Welcome to wxOCC"))
...
void MainFrame::OnCreatewxOCCCntrl(wxCommandEvent& WXUNUSED(event))
{
    wxBitmap page_bmp = wxArtProvider::GetBitmap(wxART_NORMAL_FILE, wxART_OTHER, wxSize(16, 16));
...
wxOCCWindow* MainFrame::GetActiveDocument()
{
    if(m_ntbk->GetPageCount() > 0) {
        // If it's an OpenGL Pane, then it's a child of the pane
        if(m_ntbk->GetPage(m_ntbk->GetSelection())->IsKindOf(CLASSINFO(wxOCCWindow))) {
            return (wxOCCWindow*)(m_ntbk->GetPage(m_ntbk->GetSelection()));
        }
    }
    return new wxOCCWindow(this, this->GetPosition(), this->GetSize(), 1);
}
...
void MainFrame::OnAbout(wxCommandEvent& WXUNUSED(event))
{
    ...
    info.SetName("wxOCC");
    info.SetCopyright(_("wxOCC (C) Virtex Edge Design"));
    info.AddDeveloper("R. T. Roe (rtrawr)");
    info.SetVersion(" - v. 0.3.0");
    info.SetWebSite("https://github.com/rtrawr/wxOCC");
    info.SetLicence(_("Attribution-ShareAlike 4.0 International.\n\nSee Github for Full License."));
    info.SetDescription(_("wxOCC is an implementation of OpenCasecade integrated into "
